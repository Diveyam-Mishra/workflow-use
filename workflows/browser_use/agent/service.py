...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	async def multi_act(
		...
		for i, action in enumerate(actions):
			# Store current URL before action
			current_page = await self.browser_session.get_current_page()
			...
			if i > 0:
				...
				if action.get_index() is not None:
					...
					if orig_target_hash is not None and orig_target_hash != new_target_hash:
			...
			try:
				...
				# Check for navigation AFTER action execution
				current_page_after_action = await self.browser_session.get_current_page()
				url_after_action = current_page_after_action.url

				if url_before_action != url_after_action:
					self.logger.info(
						f"Navigation detected from '{url_before_action}' to '{url_after_action}'. Resetting DOM context for next action."
					)
					# Wait for the new page to be fully loaded before proceeding
					await current_page_after_action.wait_for_load_state('load', timeout=15000)
					await asyncio.sleep(5)  # Extra wait for JS-heavy pages to settle

					# Reset the cached selector map and hashes for the new page
					cached_selector_map = await self.browser_session.get_selector_map()
					cached_path_hashes = {e.hash.branch_path_hash for e in cached_selector_map.values()}
